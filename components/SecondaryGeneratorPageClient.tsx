...
export default function SecondaryGeneratorPageClient({
  ...
  const text = translations[generatorLanguage]?.ui ?? translations.en.ui
            ...
            <div className="space-y-5">
              <section className="rounded-[28px] border border-white/10 bg-white/[0.055] p-5 shadow-[0_18px_45px_rgba(0,0,0,0.26)] backdrop-blur-xl">
                ...
                <p className={`mt-1 ${helperClassName}`}>
                  {text.secondaryInvitationDescription || 'Review the approved details that will appear on the invitation card.'}
                </p>

                <div className="mt-4 rounded-2xl border border-blue-400/20 bg-black/28 px-4 py-3 text-xs text-blue-100">
                  {(text.secondaryRequestLoaded || 'Request {requestId} loaded from Google Sheets.').replace('{requestId}', data.requestId)}
                </div>
                ...
                <div className="mt-4 space-y-4">
                  ...
                  <div className={panelClassName}>
                    <div>
                      <p className="text-sm font-semibold">
                        {text.secondaryAssignedByTeam || 'Assigned by the ISE team'}
                      </p>
                      <p className={`mt-1 text-xs ${helperClassName}`}>
                        {text.secondaryAssignedDescription || 'These values are controlled from the secondary invitation spreadsheet.'}
                      </p>
                    </div>
                    ...
                    <div className="mt-4 grid grid-cols-2 gap-3">
                      <div className={detailCardClassName}>
                        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-white/40">
                          {text.secondaryInvitationId || 'Invitation ID'}
                        </p>
                      ...
                      <div className={detailCardClassName}>
                        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-white/40">
                          {text.generatorInvitationCode || 'Invitation code'}
                        </p>
                        <p className="mt-2 break-words text-base font-semibold text-white">
                          {invitationCode || '—'}
                        </p>
                  ...
                  <label className="block">
                    <span className={labelClassName}>{text.generatorCardLanguage}</span>
                    <select
                      value={cardLanguage}
                      onChange={(event) =>
                    ...
                    <div className="mb-3 flex items-center justify-between gap-3">
                      <div>
                        <div className="text-sm font-medium text-white/70">
                          {text.secondaryCompanyLogo || 'Company logo'}
                        </div>
                        <div className="mt-1 text-xs text-white/35">
                          {text.secondaryLogoHelp || 'Upload or replace the logo for this download session. Spreadsheet logo notes are ignored unless they are real image URLs.'}
                        </div>
                      </div>
              ...
              <section className="rounded-[28px] border border-white/10 bg-white/[0.055] p-5 shadow-[0_18px_45px_rgba(0,0,0,0.26)] backdrop-blur-xl">
                <h2 className="text-xl font-semibold">{text.generatorChooseTheme}</h2>
                      ...
                      >
                        ...
                        <div className="relative">
                          <div className="text-xl font-semibold">
                            {(text.themeLabels as Partial<Record<ThemeKey, string>> | undefined)?.[key] ?? item.label}
                          </div>
                        </div>
                      </button>
                    )
          ...
          <div className="relative w-full max-w-[520px] rounded-[32px] border border-white/12 bg-[#10162b]/94 p-6 text-white shadow-[0_30px_90px_rgba(0,0,0,0.55)] backdrop-blur-2xl">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-white/45">
                  {text.downloadExportAssets || 'Export assets'}
                </p>
                <h2 className="mt-2 text-2xl font-semibold tracking-[-0.04em]">
                  {text.downloadFormatTitle || 'Download the format of your choice'}
                </h2>
                <p className="mt-2 text-sm leading-6 text-white/50">
                  {text.downloadFormatDescription || 'Choose a single format or download the complete ZIP pack.'}
                </p>
              </div>
            ...
            <div className="mt-6 grid grid-cols-2 gap-3">
              ...
              <button
                ...
                className={secondaryButtonClassName}
              >
                {text.downloadLinkedIn || 'LinkedIn'}
              </button>
              ...
              <button
                ...
                className={secondaryButtonClassName}
              >
                {text.downloadEmailBanner || 'Email Banner'}
              </button>
              ...
              >
                {text.downloadZipPack || 'ZIP Pack (all formats)'}
              </button>
            </div>
          </div>
        </div>
